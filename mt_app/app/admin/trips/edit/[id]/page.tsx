...
interface Place { place_id: string; place_name: string | null; city?: string | null }
...
const EditTripPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ description: '', duration: '', total_price: '' });
  ...
  useEffect(() => {
    const loadAll = async () => {
      try {
        ...
        setSelectedPlaceIds((trip.package_places || []).map((pp: any) => pp.place_id).filter((x: any) => !!x));
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <button
            onClick={() => router.push(`/admin/trips/${id}`)}
