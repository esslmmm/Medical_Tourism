...
const TourismService: React.FC<TourismServiceProps> = ({ TripData, appointmentDate, onGoBack }) => {
  ...
  useEffect(() => {
    if (TripData?.trips?.Trip_Routes?.length) {
      const firstTrip = TripData.trips.Trip_Routes[0].routes;
    ...
    <div className="">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8 ">
        <div className="lg:col-span-2 space-y-8">
          {/* Tabs Section */}
          <div className="sticky top-0 bg-white z-40 border-b border-gray-200 flex items-center">
            {/* Chevron Left */}
            {showLeftArrow && (
              <button
            ...
            >
              {tabs.map((tab) => (
                <button
                  key={tab}
                  className={`pb-2 border-b-2 px-2 transition-colors whitespace-nowrap text-sm md:text-base snap-start ${activeTab === tab
