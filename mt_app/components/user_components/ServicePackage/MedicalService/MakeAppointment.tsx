    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <button
          key={day}
          disabled={isPast}
          onClick={() => { if (!isPast) { setSelectedDate(currentDate); setShowCalendar(false); } }}
          className={`h-10 rounded-lg flex items-center justify-center font-medium transition-colors text-black
        ...
        {showDateSection && (
          <>
            <div className="relative">
              ...
              <div ref={scrollDateRef} className="flex gap-2 overflow-x-auto scrollbar-hide px-8">
                {dates.map((date, index) => {
                  ...
                  return (
                    <div key={index} onClick={() => setSelectedDate(date)} className={`flex-shrink-0 w-20 border rounded-2xl p-3 text-center cursor-pointer transition-all ${isSelected ? 'border-teal-500 bg-teal-50 border-2' : 'border-gray-300 hover:border-teal-300'}`}>
            ...
            <div className="mt-4 p-4 bg-gray-50 rounded-lg">
              ...
              <p className="text-lg font-semibold text-gray-900">{selectedDate.toLocaleDateString('en-US', { weekday:'long', year:'numeric', month:'long', day:'numeric' })}</p>
            </div>
          </>
        )}
      </div>
          ...
          <div className="relative">
            ...
            <div ref={scrollTimeRef} className="flex gap-2 overflow-x-auto scrollbar-hide px-8">
              {timeSlots.map((time, index) => (
                <button key={index} onClick={() => setSelectedTime(time)} className={`flex-shrink-0 py-3 px-5 rounded-xl text-sm font-bold transition-colors ${selectedTime === time ? 'bg-teal-100 text-teal-600' : 'text-gray-400 hover:bg-gray-100'}`}>{time}</button>
              ))}
            </div>
        ...
        <div className="fixed inset-0 backdrop-blur-xs bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              {timeSlots.map((time, index) => (
                <button key={index} onClick={() => { setSelectedTime(time); setShowTimeModal(false); }} className={`py-3 rounded-lg font-semibold transition-colors ${selectedTime === time ? 'bg-teal-100 text-teal-600' : 'bg-gray-50 text-gray-700 hover:bg-gray-100'}`}>{time}</button>
              ))}
            </div>
          </div>
        </div>
          ...
          <>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center bg-white rounded-full border border-gray-200">
                <button onClick={() => setAdults(Math.max(0, adults - 1))} className="p-3 text-gray-400 hover:text-gray-600"><Minus className="w-4 h-4"/></button>
                <span className="px-4 py-2 font-medium text-emerald-500">{adults}</span>
                <button onClick={() => setAdults(adults + 1)} className="p-3 text-gray-400 hover:text-gray-600"><Plus className="w-4 h-4"/></button>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center bg-white rounded-full border border-gray-200">
                <button onClick={() => setChildren(Math.max(0, children - 1))} className="p-3 text-gray-400 hover:text-gray-600"><Minus className="w-4 h-4"/></button>
                <span className="px-4 py-2 font-medium text-emerald-500">{children}</span>
                <button onClick={() => setChildren(children + 1)} className="p-3 text-gray-400 hover:text-gray-600"><Plus className="w-4 h-4"/></button>
              </div>
            </div>
          </>
        )}
